...
void requestMissile(int mapX, int mapY, acc user, char userSea[mapX][mapY]){
    if ( user.coin >= 100) {
        user.coin -= 100;
        int n;
        char pos;
        int res;
        coord target;
        printf("Commander this is captain Alfa 12, Give me a line to smoke :\n");
        scanf("%d %c", &n, &pos);
        ...
        if (pos == 'V' || pos == 'v') {
            ...
            for (int a = 0; a < mapY; a++) {
                ...
                res = shoot(mapX, mapY, userSea, target);
                if (res == 1) break;
            }
...
drawBattleMap(mapX,mapY,userSea);
        } else {
            printf("\n\nNot enough Scores!\n\n");
        }
...
void battleBase(int mapX, int mapY, char user1sea[mapX][mapY], char user2sea[mapX][mapY],
    ...
    while (1) { //ends when a list reaches end
        //115 longer
        Sleep(3500); // to see the res of attack
        system("clear");
        setColor('y');
        ...
        //saving
        if (inTarget.x == -1)  {
            printf("\n\n Please Enter a name for your Save file (MAX 50 characters) \n");
            char name[50];
            scanf("%s", name);
            saveBattle(name, mapX,mapY,user1sea,user2sea,pHeadUser1,pHeadUser2,user1,user2,rol);
            printf("\n %s has been saved \n",name);
            saveScreen();
            return;
        }
        if(inTarget.x == -2){
            if (rol == 1) requestMissile(mapX,mapY,*user1,user2sea);
            else requestMissile(mapX,mapY,*user2,user1sea);
